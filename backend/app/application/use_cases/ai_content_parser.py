import json
from logging import Logger
from openai import AsyncOpenAI
from typing import List, Literal, TypedDict
from openai.types import ChatModel

from app.config.prompts import StructuredReply


class _ConversationContent(TypedDict):
    role: Literal["system", "user", "assistant"]
    content: str


class ArticleContentParserUseCase:
    def __init__(self, logger: Logger, client: AsyncOpenAI,
                 model: ChatModel = "gpt-4o-mini", system_prompt: str = ""):
        self.logger = logger
        self.client = client
        self.open_ai_model = model
        self.logger.info(
            "Appending system prompt to the conversation history.")
        self._conversation_history: List[_ConversationContent] = [
            {"role": "system", "content": system_prompt}]

    def __try_parse_json(self, text: str) -> StructuredReply | None:
        self.logger.info(
            "Trying to parse AI reply in a structured JSON format.")
        try:
            return json.loads(text)
        except Exception:
            return None

    async def execute(self, message: str):
        """
        This function takes the message and process with
        the ai model to get the task suggestion.

        Each interaction is stored in a conversation history.

        :param message: The last message to be processed by the AI.
        :type message: str

        :return: The AI reply as a typed dict object.
        :rtype: StructuredReply OR None

        :raise RuntimeError: OpenAI API error.
        """
        self.logger.info("Appending user message to conversation history")
        self.logger.debug(f"User message: {message}")
        self._conversation_history.append(
            {"role": "user", "content": message})

        try:
            self.logger.info(
                "Calling OpenAI with conversation history to get AI reply")
            completion = await self.client.chat.completions.create(
                model=self.open_ai_model,
                messages=self._conversation_history,
                temperature=0.2,
            )
        except Exception as e:
            self.logger.error(f"Error calling OpenAI API: {e}")
            raise RuntimeError(f"OpenAI API error: {e}")

        # Save AI reply in conversation history
        ai_reply = completion.choices[0].message.content or ""
        if ai_reply:
            self.logger.info("Appending AI reply to conversation history")
            self.logger.debug(f"AI reply: {ai_reply}")
            self._conversation_history.append(
                {"role": "assistant", "content": ai_reply})

        return self.__try_parse_json(ai_reply)
